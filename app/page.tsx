...
// ─── Detect reduced-motion only (NOT screen size — plasma shows on all screens) ───
function useLowMotion() {
  const [lowMotion, setLowMotion] = useState(false); // default false = show plasma

  useEffect(() => {
    const mq = window.matchMedia("(prefers-reduced-motion: reduce)");
    const check = () => setLowMotion(mq.matches); // only disable if user explicitly requested
...
/* ─────────────────────────────────────────
   Reusable section badge — memoized
───────────────────────────────────────── */
...
/* ─────────────────────────────────────────
   Parallax orbs — isolated component so
   mouse state changes DON'T re-render the hero
───────────────────────────────────────── */
const ParallaxOrbs = memo(function ParallaxOrbs() {
  // Use a ref for mouse position to avoid re-renders; update CSS vars directly
  ...
  return (
    <div className="flex flex-col overflow-x-hidden">
      ...
      >
        ...
        {/* Parallax orbs — isolated, no hero re-renders on mouse */}
        {!lowMotion && <ParallaxOrbs />}
          ...
          <div className="mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-10">
            ...
            >
              ...
              >
                ...
                <svg className="h-4 w-4 transition-transform group-hover:translate-x-1 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              >
                ...
                <svg className="h-4 w-4 transition-transform group-hover:translate-x-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            >
              {statItems.map(({ num, label, accent }) => (
                <div
                  key={label}
                  className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-white/[0.08] hover:-translate-y-0.5 hover:shadow-xl sm:p-6"
                >
                  <div
      ...
      <div className="mx-auto w-full max-w-screen-xl space-y-24 px-4 pt-20 pb-24 sm:px-6 lg:px-10 xl:px-16">
        ...
        <section className="space-y-10">
          <div className="space-y-3 text-center">
            ...
            <h2 className="font-extrabold tracking-tight text-white" style={{ fontSize: "clamp(2rem,5vw,3.25rem)" }}>
              How We Work
            </h2>
        ...
        >
          ...
          <div className="relative space-y-6 text-center">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-emerald-500/20 bg-emerald-500/10 text-2xl">
              🚀
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
              {["IoT & Embedded", "Web Platforms", "Mobile Apps", "Cloud & DevOps"].map((tag) => (
                <span key={tag} className="rounded-full border border-emerald-500/20 bg-emerald-500/8 px-3 py-1 text-xs font-medium text-emerald-400">
                  {tag}
                </span>
              ))}
            </div>
            <div className="pt-4">
              ...
              >
                <span className="absolute inset-0 bg-white/10 opacity-0 transition-opacity group-hover:opacity-100" />
                Contact EcoBridges
                <svg className="h-5 w-5 shrink-0 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
